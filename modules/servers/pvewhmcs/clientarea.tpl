...
<div class="pve-client-area">
	...
	<div class="pve-header-panel">
		<div class="pve-status-section">
			...
			<div class="pve-vm-icons">
				<img src="./modules/servers/pvewhmcs/img/{$pve_vm_config['vtype']}.png" alt="{$pve_vm_config['vtype']}" title="Type: {$pve_vm_config['vtype']}"/>
				<img src="./modules/servers/pvewhmcs/img/os/{$pve_vm_config['ostype']}.png" alt="{$pve_vm_config['ostype']}" title="OS: {$pve_vm_config['ostype']}"/>
			</div>
			
			{* Status Badge *}
			<div class="pve-status-badge">
				<img src="./modules/servers/pvewhmcs/img/{$pve_vm_status['status']}.png" alt="{$pve_vm_status['status']}"/>
				<span class="status-text">{$pve_vm_status['status']}</span>
				<span class="uptime-text">Up {$pve_vm_status['uptime']}</span>
			</div>
			...
			<div class="pve-gauges">
				...
				<div class="pve-gauge-item">
					<div id="c1" class="circle" data-percent="{$pve_vm_status['cpu']}"></div>
					<strong>CPU</strong>
				</div>
				<div class="pve-gauge-item">
					<div id="c2" class="circle" data-percent="{$pve_vm_status['memusepercent']}"></div>
					<strong>RAM</strong>
				</div>
				<div class="pve-gauge-item">
					<div id="c3" class="circle" data-percent="{$pve_vm_status['diskusepercent']}"></div>
					<strong>Disk</strong>
				</div>
				<div class="pve-gauge-item">
					<div id="c4" class="circle" data-percent="{$pve_vm_status['swapusepercent']}"></div>
					<strong>Swap</strong>
				</div>
			</div>
		</div>
	...
	<table class="pve-specs-table">
		<tr>
			...
			<td><span class="spec-value">{$pve_vm_config['memory']}MB</span></td>
		</tr>
		<tr>
			<td><span class="spec-label">Compute</span> <span class="spec-sublabel">(CPU)</span></td>
			<td>
				<span class="spec-value">{$pve_vm_config['cores']} core(s)</span>
				<div class="spec-detail">on {$pve_vm_config['sockets']} socket(s)</div>
			</td>
		</tr>
		<tr>
			<td><span class="spec-label">Storage</span> <span class="spec-sublabel">(SSD/HDD)</span></td>
			<td>
				{if $pve_vm_config['rootfs']}
					{assign var="rootfs_parts" value=","|explode:$pve_vm_config['rootfs']}
					...
					{/foreach}
				{/if}
				{if $pve_vm_config['ide0']}
					{assign var="ide0_parts" value=","|explode:$pve_vm_config['ide0']}
					...
					{/foreach}
				{/if}
				{if $pve_vm_config['scsi0']}
					{assign var="scsi0_parts" value=","|explode:$pve_vm_config['scsi0']}
					...
					{/foreach}
				{/if}
				{if $pve_vm_config['virtio0']}
					{assign var="virtio0_parts" value=","|explode:$pve_vm_config['virtio0']}
		...
		<tr>
			...
			<td><span class="spec-value">{($pve_vm_config['boot']|replace:'order=':''|replace:';':' → ')}</span></td>
		</tr>
		<tr>
			<td><span class="spec-label">IPv4</span> <span class="spec-sublabel">(Networking)</span></td>
			<td>
				<span class="spec-value">{$pve_vm_config['ipv4']}</span>
				<div class="spec-detail">Mask: {$pve_vm_config['netmask4']} &bull; Gateway: {$pve_vm_config['gateway4']}</div>
			</td>
		</tr>
		<tr>
			<td><span class="spec-label">IP Config</span> <span class="spec-sublabel">(IPv4/v6)</span></td>
			<td>
				{if $pve_vm_config['ipconfig0']}<div class="spec-detail"><strong>NIC #0:</strong> {($pve_vm_config['ipconfig0']|replace:',':' &bull; '|replace:'=':': ')}</div>{/if}
				{if $pve_vm_config['ipconfig1']}<div class="spec-detail"><strong>NIC #1:</strong> {($pve_vm_config['ipconfig1']|replace:',':' &bull; '|replace:'=':': ')}</div>{/if}
			</td>
		</tr>
		<tr>
			<td><span class="spec-label">NIC #0</span> <span class="spec-sublabel">(Primary)</span></td>
			<td>
				{assign var="net0_parts" value=","|explode:$pve_vm_config['net0']}
		...
		<tr>
			...
			<td><span class="spec-detail"><strong>On-boot?</strong> {if $pve_vm_config['onboot']}Yes!{else}No (Contact Support){/if}</span></td>
		</tr>
		{if $pve_vm_config['sshkeys']}
		<tr>
			<td><span class="spec-label">SSH Keys</span> <span class="spec-sublabel">(Public)</span></td>
			<td><div class="spec-detail" style="word-break:break-all;">{$pve_vm_config['sshkeys']}</div></td>
		</tr>
		{/if}
		<tr>
			<td><span class="spec-label">Kernel</span> <span class="spec-sublabel">(OS)</span></td>
			<td><span class="spec-value">{$pve_vm_config['ostype']}</span></td>
		</tr>
	</table>
		...
		<div class="pve-stats-content tab-content">
			<div id="dailystat" class="tab-pane active">
				<div class="pve-graphs-grid">
					<img src="data:image/png;base64,{$pve_vm_statistics['cpu']['day']}" alt="CPU (Daily)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['mem']['day']}" alt="Memory (Daily)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['netinout']['day']}" alt="Network I/O (Daily)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['diskrw']['day']}" alt="Disk I/O (Daily)"/>
				</div>
			</div>
			<div id="weeklystat" class="tab-pane">
				<div class="pve-graphs-grid">
					<img src="data:image/png;base64,{$pve_vm_statistics['cpu']['week']}" alt="CPU (Weekly)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['mem']['week']}" alt="Memory (Weekly)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['netinout']['week']}" alt="Network I/O (Weekly)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['diskrw']['week']}" alt="Disk I/O (Weekly)"/>
				</div>
			</div>
			<div id="monthlystat" class="tab-pane">
				<div class="pve-graphs-grid">
					<img src="data:image/png;base64,{$pve_vm_statistics['cpu']['month']}" alt="CPU (Monthly)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['mem']['month']}" alt="Memory (Monthly)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['netinout']['month']}" alt="Network I/O (Monthly)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['diskrw']['month']}" alt="Disk I/O (Monthly)"/>
				</div>
			</div>
			<div id="yearlystat" class="tab-pane">
				<div class="pve-graphs-grid">
					<img src="data:image/png;base64,{$pve_vm_statistics['cpu']['year']}" alt="CPU (Yearly)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['mem']['year']}" alt="Memory (Yearly)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['netinout']['year']}" alt="Network I/O (Yearly)"/>
					<img src="data:image/png;base64,{$pve_vm_statistics['diskrw']['year']}" alt="Disk I/O (Yearly)"/>
				</div>
			</div>
		</div>
		{else}
